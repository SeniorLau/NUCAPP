...
with tab_main:
    ...
    st.markdown("**Workaround:** Manually enter the x-values (time in minutes) where nucleation occurs.")
    ...
    ax.set_title(f'Temperature Curves with {ci_level}% Confidence Intervals')
    ax.grid(True)
    ax.set_ylim([-90, 20])
    ax.set_xlim([0, XLIM])
    ax.legend()
    ...
    ax.set_xlabel('Time (s)')
    ax.set_ylabel('Flow')
    ax.set_title(f'Flow Curves with {ci_level}% Confidence Intervals')
    ax.grid(True)
    ax.legend()
    st.pyplot(fig_flow)
